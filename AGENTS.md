...
Write code that signs, sends transactions or handles private keys (sole exception: the devnet carve-out below).
...
Devnet signing carve-out (maintainer-approved, Phase 8; needed again by the Phase 10 devnet end-to-end test). Signing and sending transactions is allowed only when all of these hold: (1) the code lives in scripts/ (the raw signing APIs only in scripts/lib/devnet.ts); (2) it runs only on devnet, behind a genesis-hash guard that refuses any other cluster before a key exists; (3) it uses only in-memory throwaway keys, generated by the script, never written to disk, logged, committed or accepted as input (only public keys are printed); (4) it is never in packages/core, packages/cli, apps/web or apps/rpc-proxy, and nothing shipped imports a signing library. packages/core/src/signing-boundary.test.ts fails if signing code appears outside scripts/. This carve-out does not apply to the product: Vigil itself stays read-only.
