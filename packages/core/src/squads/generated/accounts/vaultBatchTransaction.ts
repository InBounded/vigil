...
export const VAULT_BATCH_TRANSACTION_DISCRIMINATOR: ReadonlyUint8Array = new Uint8Array([
  196, 121, 46, 36, 12, 19, 252, 7,
]);

export function getVaultBatchTransactionDiscriminatorBytes(): ReadonlyUint8Array {
  return fixEncoderSize(getBytesEncoder(), 8).encode(VAULT_BATCH_TRANSACTION_DISCRIMINATOR);
}
...
export function getVaultBatchTransactionEncoder(): Encoder<VaultBatchTransactionArgs> {
  return transformEncoder(
    getStructEncoder([
      ...
      ["bump", getU8Encoder()],
      ["ephemeralSignerBumps", addEncoderSizePrefix(getBytesEncoder(), getU32Encoder())],
...
export function getVaultBatchTransactionDecoder(): Decoder<VaultBatchTransaction> {
  return getStructDecoder([
    ...
    ["bump", getU8Decoder()],
    ["ephemeralSignerBumps", addDecoderSizePrefix(getBytesDecoder(), getU32Decoder())],
...
> {
  return combineCodec(getVaultBatchTransactionEncoder(), getVaultBatchTransactionDecoder());
}
...
): Account<VaultBatchTransaction, TAddress> | MaybeAccount<VaultBatchTransaction, TAddress> {
...
export async function fetchVaultBatchTransaction<TAddress extends string = string>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
  config?: FetchAccountConfig,
): Promise<Account<VaultBatchTransaction, TAddress>> {
  const maybeAccount = await fetchMaybeVaultBatchTransaction(rpc, address, config);
  assertAccountExists(maybeAccount);
  return maybeAccount;
}

export async function fetchMaybeVaultBatchTransaction<TAddress extends string = string>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
  config?: FetchAccountConfig,
...
export async function fetchAllVaultBatchTransaction(
  ...
  const maybeAccounts = await fetchAllMaybeVaultBatchTransaction(rpc, addresses, config);
...
export async function fetchAllMaybeVaultBatchTransaction(
  ...
  return maybeAccounts.map((maybeAccount) => decodeVaultBatchTransaction(maybeAccount));
}
