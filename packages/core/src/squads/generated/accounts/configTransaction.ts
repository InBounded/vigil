...
export const CONFIG_TRANSACTION_DISCRIMINATOR: ReadonlyUint8Array = new Uint8Array([
  94, 8, 4, 35, 113, 139, 139, 112,
]);

export function getConfigTransactionDiscriminatorBytes(): ReadonlyUint8Array {
  return fixEncoderSize(getBytesEncoder(), 8).encode(CONFIG_TRANSACTION_DISCRIMINATOR);
}
...
export function getConfigTransactionCodec(): Codec<ConfigTransactionArgs, ConfigTransaction> {
  return combineCodec(getConfigTransactionEncoder(), getConfigTransactionDecoder());
}
...
): Account<ConfigTransaction, TAddress> | MaybeAccount<ConfigTransaction, TAddress> {
...
export async function fetchMaybeConfigTransaction<TAddress extends string = string>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
  config?: FetchAccountConfig,
...
export async function fetchAllConfigTransaction(
  ...
  const maybeAccounts = await fetchAllMaybeConfigTransaction(rpc, addresses, config);
...
export async function fetchAllMaybeConfigTransaction(
  ...
  return maybeAccounts.map((maybeAccount) => decodeConfigTransaction(maybeAccount));
}
