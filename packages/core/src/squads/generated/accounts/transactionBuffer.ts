...
export const TRANSACTION_BUFFER_DISCRIMINATOR: ReadonlyUint8Array = new Uint8Array([
  90, 36, 35, 219, 93, 225, 110, 96,
]);

export function getTransactionBufferDiscriminatorBytes(): ReadonlyUint8Array {
  return fixEncoderSize(getBytesEncoder(), 8).encode(TRANSACTION_BUFFER_DISCRIMINATOR);
}
...
export function getTransactionBufferCodec(): Codec<TransactionBufferArgs, TransactionBuffer> {
  return combineCodec(getTransactionBufferEncoder(), getTransactionBufferDecoder());
}
...
): Account<TransactionBuffer, TAddress> | MaybeAccount<TransactionBuffer, TAddress> {
...
export async function fetchMaybeTransactionBuffer<TAddress extends string = string>(
  rpc: Parameters<typeof fetchEncodedAccount>[0],
  address: Address<TAddress>,
  config?: FetchAccountConfig,
