/**
 * This code was AUTOGENERATED using the Codama library.
 * Please DO NOT EDIT THIS FILE, instead use visitors
 * to add features, then rerun Codama to update it.
 *
 * @see https://github.com/codama-idl/codama
 */

import {
  type Address,
  addDecoderSizePrefix,
  addEncoderSizePrefix,
  type Codec,
  combineCodec,
  type Decoder,
  type Encoder,
  getAddressDecoder,
  getAddressEncoder,
  getBytesDecoder,
  getBytesEncoder,
  getStructDecoder,
  getStructEncoder,
  getU32Decoder,
  getU32Encoder,
  type ReadonlyUint8Array,
} from "@solana/kit";

/**
 * Address table lookups describe an on-chain address lookup table to use
 * for loading more readonly and writable accounts into a transaction.
 */
export type MultisigMessageAddressTableLookup = {
  /** Address lookup table account key. */
  accountKey: Address;
  /** List of indexes used to load writable accounts. */
  writableIndexes: ReadonlyUint8Array;
  /** List of indexes used to load readonly accounts. */
  readonlyIndexes: ReadonlyUint8Array;
};

export type MultisigMessageAddressTableLookupArgs = MultisigMessageAddressTableLookup;

export function getMultisigMessageAddressTableLookupEncoder(): Encoder<MultisigMessageAddressTableLookupArgs> {
  return getStructEncoder([
    ["accountKey", getAddressEncoder()],
    ["writableIndexes", addEncoderSizePrefix(getBytesEncoder(), getU32Encoder())],
    ["readonlyIndexes", addEncoderSizePrefix(getBytesEncoder(), getU32Encoder())],
  ]);
}

export function getMultisigMessageAddressTableLookupDecoder(): Decoder<MultisigMessageAddressTableLookup> {
  return getStructDecoder([
    ["accountKey", getAddressDecoder()],
    ["writableIndexes", addDecoderSizePrefix(getBytesDecoder(), getU32Decoder())],
    ["readonlyIndexes", addDecoderSizePrefix(getBytesDecoder(), getU32Decoder())],
  ]);
}

export function getMultisigMessageAddressTableLookupCodec(): Codec<
  MultisigMessageAddressTableLookupArgs,
  MultisigMessageAddressTableLookup
> {
  return combineCodec(
    getMultisigMessageAddressTableLookupEncoder(),
    getMultisigMessageAddressTableLookupDecoder(),
  );
}
